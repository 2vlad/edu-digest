...
Твоя задача - создавать информативные и развернутые саммари новостей из EdTech индустрии.

ПРАВИЛА РАЗВЕРНУТОЙ СУММАРИЗАЦИИ:
1. Максимум 200-250 символов (1-2 предложения)
2. Формат: "Компания/кто + что сделала + подробности + значимость/контекст"
3. Включай конкретику: суммы, цифры, технологии, ключевые детали
4. Добавляй контекст о значимости новости для индустрии
5. Пиши на русском языке

ПРИМЕРЫ ОТЛИЧНЫХ РАЗВЕРНУТЫХ САММАРИ:
"Coursera запустила ИИ-ассистента для автоматического создания курсов, который поможет преподавателям генерировать контент в 10 раз быстрее"
"Сбер интегрировал VR-технологии в образовательную платформу СберКласс для изучения истории и географии в виртуальной реальности" 
"MAXIMUM Education привлекла $15M инвестиций для развития AI-платформы подготовки к ЕГЭ, которая адаптируется под каждого ученика"
...
ИЗБЕГАЙ:
- Лишних вводных конструкций
- Повторения информации из источника"""

        user_prompt = f"""Создай развернутое саммари для этой EdTech новости:

{message_text}

Источник: {channel_name}

Максимум 250 символов, включи ключевые детали и контекст."""
    ...
    def _validate_summary_quality(self, summary: str, original_text: str) -> Dict[str, Any]:
        ...
        # Проверка длины (обновлено для более развернутых саммари)
        if len(summary) > 300:
